...
impl<'a> Lexer<'a> {
    ...
    fn lex_identifier_or_keyword(&mut self) -> Result<Token, LexError> {
        ...
        let kind = match id.as_str() {
            ...
            "Pra" | "pra" => TokenKind::Pra,
            "Para" | "para" => TokenKind::Para,
            "Apa" | "apa" => TokenKind::Apa,
            "Sam" => TokenKind::Sam,
            "Anu" => TokenKind::Anu,
            "Ava" => TokenKind::Ava,
            "Nis" => TokenKind::Nis,
...
"dhātu" | "Dhātu" => TokenKind::Dhātu,
             ...
             "samyoga" | "Samyoga" => TokenKind::Samyoga,
             "prapti" | "Prapti" => TokenKind::Prapti,
             "duta" | "Duta" => TokenKind::Duta,
             "banaa" | "Banaa" => TokenKind::Banaa,
             "bhej" | "Bhej" => TokenKind::Bhej,
             ...
             "karo" | "Karo" => TokenKind::Karo,
              "manas" | "Manas" => TokenKind::Manas,
               ...
               // --- PARIṬṢĀ (TESTING) KEYWORDS ---
               "parikshaa" | "Parikshaa" => TokenKind::Parikshaa,
               "nigamana" | "Nigamana" => TokenKind::Nigamana,
               "sadrishya-nigamana" | "Sadrishya-nigamana" => TokenKind::SadrishyaNigamana,
...
mod tests {
    ...
    // --- CONCURRENCY KEYWORD TESTS ---

    #[test]
    fn test_samyoga_keyword() {
        let mut lexer = Lexer::new("samyoga");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Samyoga);
    }
    ...
    #[test]
    fn test_prapti_keyword() {
        let mut lexer = Lexer::new("prapti");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Prapti);
    }
    ...
    #[test]
    fn test_duta_keyword() {
        let mut lexer = Lexer::new("duta");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Duta);
    }
    ...
    #[test]
    fn test_banaa_keyword() {
        let mut lexer = Lexer::new("banaa");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Banaa);
    }
    ...
    #[test]
    fn test_bhej_keyword() {
        let mut lexer = Lexer::new("bhej");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Bhej);
    }
    ...
    #[test]
    fn test_grahan_keyword() {
        let mut lexer = Lexer::new("grahan");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Grahan);
    }
    ...
    #[test]
    fn test_karo_keyword() {
        let mut lexer = Lexer::new("karo");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Karo);
    }
    ...
    #[test]
    fn test_manas_keyword() {
        let mut lexer = Lexer::new("manas");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Manas);
    }
    ...
    #[test]
    fn test_manas_keyword_substring_not_keyword() {
        let mut lexer = Lexer::new("manasah");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Naama("manasah".to_string()));
    }

    #[test]
    fn test_sandesha_keyword() {
        let mut lexer = Lexer::new("sandesha");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Sandesha);
    }
    ...
    #[test]
    fn test_concurrency_keywords_not_substrings() {
        let mut lexer = Lexer::new("samyogah praptih dutah");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
    ...
    // --- PARIṬṢĀ (TESTING) KEYWORD TESTS ---

    #[test]
    fn test_parikshaa_keyword() {
        let mut lexer = Lexer::new("parikshaa");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Parikshaa);
    }
    ...
    #[test]
    fn test_nigamana_keyword() {
        let mut lexer = Lexer::new("nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Nigamana);
    }
    ...
    #[test]
    fn test_sadrishya_nigamana_keyword() {
        let mut lexer = Lexer::new("sadrishya-nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::SadrishyaNigamana);
    }
    ...
    #[test]
    fn test_sadrishya_nigamana_not_split() {
        let mut lexer = Lexer::new("sadrishya-nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
    ...
    #[test]
    fn test_sadrishya_nigamana_not_identifier_minus() {
        let mut lexer = Lexer::new("sadrishya-nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert!(matches!(tokens[0].kind, TokenKind::SadrishyaNigamana));
        assert!(!matches!(tokens[0].kind, TokenKind::Naama(_)));
    }

    #[test]
    fn test_asadrishya_nigamana_keyword() {
        let mut lexer = Lexer::new("asadrishya-nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::AsadrishyaNigamana);
    }
    ...
    #[test]
    fn test_asadrishya_nigamana_not_split() {
        let mut lexer = Lexer::new("asadrishya-nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens.len(), 2);
        assert_eq!(tokens[0].kind, TokenKind::AsadrishyaNigamana);
    ...
    #[test]
    fn test_asadrishya_nigamana_not_identifier_minus() {
        let mut lexer = Lexer::new("asadrishya-nigamana");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert!(matches!(tokens[0].kind, TokenKind::AsadrishyaNigamana));
        assert!(!matches!(tokens[0].kind, TokenKind::Naama(_)));
    }

    #[test]
    fn test_tarka_keyword() {
        let mut lexer = Lexer::new("tarka");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Tarka);
    }
    ...
    // --- DOCUMENTATION (ĀRṢA-VYĀKHYĀ) KEYWORD TESTS ---

    #[test]
    fn test_vritti_keyword() {
        let mut lexer = Lexer::new("vritti");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Vritti);
    }
    ...
    #[test]
    fn test_bhashya_keyword() {
        let mut lexer = Lexer::new("bhashya");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Bhashya);
    }
    ...
    #[test]
    fn test_tippani_keyword() {
        let mut lexer = Lexer::new("tippani");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Tippani);
    }
    ...
    #[test]
    fn test_vritti_statement() {
        let mut lexer = Lexer::new("vritti \"some text\" ।");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Vritti);
        assert_eq!(tokens[1].kind, TokenKind::VaakLiteral("some text".to_string()));
        assert_eq!(tokens[2].kind, TokenKind::Danda);
    }

    #[test]
    fn test_bhashya_statement() {
        let mut lexer = Lexer::new("bhashya \"module docs\" ।");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Bhashya);
        assert_eq!(tokens[1].kind, TokenKind::VaakLiteral("module docs".to_string()));
        assert_eq!(tokens[2].kind, TokenKind::Danda);
    }

    #[test]
    fn test_tippani_statement() {
        let mut lexer = Lexer::new("tippani \"note\" para naam ।");
        let tokens = lexer.tokenize(SandhiMode::Off).unwrap();
        assert_eq!(tokens[0].kind, TokenKind::Tippani);
        assert_eq!(tokens[1].kind, TokenKind::VaakLiteral("note".to_string()));
