...
impl<'ctx> IrEmitter<'ctx> {
    ...
    fn declare_vaak_stdlib(&mut self) {
        ...
        self.module
            .add_function("__devvani_vaak_yoga", yoga_fn_type, None);

        // __devvani_vaak_parimana(i8*) -> i64
        let parimana_fn_type = i64_type.fn_type(&[i8_ptr_type.into()], false);
        self.module
            .add_function("__devvani_vaak_parimana", parimana_fn_type, None);

        // __devvani_vaak_khanda(i8*, i64, i64) -> i8*
        let khanda_fn_type = i8_ptr_type.fn_type(
            &[i8_ptr_type.into(), i64_type.into(), i64_type.into()],
            false,
        );
        self.module
            .add_function("__devvani_vaak_khanda", khanda_fn_type, None);

        // __devvani_vaak_mukta(i8*) -> void
        let mukta_fn_type = self
            .context
            .void_type()
            .fn_type(&[i8_ptr_type.into()], false);
        self.module
            .add_function("__devvani_vaak_mukta", mukta_fn_type, None);
    }
    ...
    fn compile_node(
        ...
        match node {
            ...
            ASTNode::Nama { base, .. } => {
                let val = self.variables.get(base).ok_or_else(|| {
                    DevvaniLLVMError::CodeGenError(format!("Unknown variable: {}", base))
                })?;
                Ok(Some(*val))
            }
            ...
            ASTNode::VaakLiteral { value, .. } => {
                let val = self
                    .builder
                    .build_global_string_ptr(value, "vaak_lit")
                    .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
            ...
            ASTNode::VaakNode {
                ...
                let ptr = self
                    .builder
                    .build_alloca(i8_ptr_type, naama)
                    .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
                self.builder
                    .build_store(ptr, val)
                    .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
                ...
                if *karaka == KarakaRole::Karta {
                    self.vaak_ownership_map
                        .insert(naama.clone(), VaakOwnership::Karta);
                } else if *karaka == KarakaRole::Karana {
                    self.vaak_ownership_map
                        .insert(naama.clone(), VaakOwnership::Karana);
                }

                if *karaka == KarakaRole::Apadana {
                    self.vaak_ownership_map
                        .insert(naama.clone(), VaakOwnership::Apadana);
                }
            ...
            ASTNode::VaakYogaNode {
                ...
                let left = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("VaakYoga: no vama".into()))?;
                let right = self.compile_node(dakshina)?.ok_or_else(|| {
                    DevvaniLLVMError::CodeGenError("VaakYoga: no dakshina".into())
                })?;

                let vaak_yoga_fn = self
                    .module
                    .get_function("__devvani_vaak_yoga")
                    .unwrap_or_else(|| {
                        let fn_type =
                            i8_ptr_type.fn_type(&[i8_ptr_type.into(), i8_ptr_type.into()], false);
                        self.module
                            .add_function("__devvani_vaak_yoga", fn_type, None)
                    });

                let call = self
                    .builder
                    .build_call(vaak_yoga_fn, &[left.into(), right.into()], "yoga_result")
                    .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
            ...
            ASTNode::DharaNode { naamas, mulya, .. } => {
                ...
                self.variables.insert(naamas[0].clone(), val);
                Ok(Some(val))
            }
            ASTNode::YogaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Yoga: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Yoga: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
                            .build_int_add(l, r, "addtmp")
                            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
                            .into(),
                    )),
                    (BasicValueEnum::FloatValue(l), BasicValueEnum::FloatValue(r)) => Ok(Some(
                        self.builder
            ...
            ASTNode::ViyogaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Viyoga: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Viyoga: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
                    ...
                    _ => Err(DevvaniLLVMError::CodeGenError(
                        "Viyoga: type mismatch".into(),
                    )),
                }
            }
            ASTNode::GunaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Guna: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Guna: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
            ...
            ASTNode::BhagaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Bhaga: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Bhaga: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
            ...
            ASTNode::SamaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Sama: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Sama: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
            ...
            ASTNode::AsamaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Asama: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Asama: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
                    ...
                    (BasicValueEnum::FloatValue(l), BasicValueEnum::FloatValue(r)) => Ok(Some(
                        self.builder
                            .build_float_compare(inkwell::FloatPredicate::ONE, l, r, "fnetmp")
                            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
                            .into(),
                    )),
                    _ => Err(DevvaniLLVMError::CodeGenError(
                        "Asama: type mismatch".into(),
                    )),
                }
            }
            ASTNode::NyuunaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Nyuuna: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Nyuuna: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
                            .build_int_compare(inkwell::IntPredicate::SLT, l, r, "slttmp")
                            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
                            .into(),
                    )),
                    (BasicValueEnum::FloatValue(l), BasicValueEnum::FloatValue(r)) => Ok(Some(
                        self.builder
                            .build_float_compare(inkwell::FloatPredicate::OLT, l, r, "folttmp")
                            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
                            .into(),
                    )),
                    _ => Err(DevvaniLLVMError::CodeGenError(
                        "Nyuuna: type mismatch".into(),
                    )),
                }
            }
            ASTNode::AdhikaNode { vama, dakshina } => {
                let lhs = self
                    .compile_node(vama)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Adhika: no vama".into()))?;
                let rhs = self
                    .compile_node(dakshina)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Adhika: no dakshina".into()))?;
                match (lhs, rhs) {
                    (BasicValueEnum::IntValue(l), BasicValueEnum::IntValue(r)) => Ok(Some(
                        self.builder
                            .build_int_compare(inkwell::IntPredicate::SGT, l, r, "sgttmp")
                            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
                            .into(),
                    )),
                    (BasicValueEnum::FloatValue(l), BasicValueEnum::FloatValue(r)) => Ok(Some(
                        self.builder
                            .build_float_compare(inkwell::FloatPredicate::OGT, l, r, "fogttmp")
                            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
                            .into(),
                    )),
                    _ => Err(DevvaniLLVMError::CodeGenError(
                        "Adhika: type mismatch".into(),
                    )),
                }
            }
            ASTNode::VadatiNode { mulya } => {
                let val = self
                    .compile_node(mulya)?
                    .ok_or_else(|| DevvaniLLVMError::CodeGenError("Vadati: no mulya".into()))?;
            ...
            ASTNode::YavatNode { sthiti, shareera } => self.compile_yavat(sthiti, shareera),
            ASTNode::PunahNode { varam, shareera } => self.compile_punah(varam, shareera),
            ASTNode::KriyaCall { kriya, karma, .. } => self.compile_function_call(kriya, karma),
            _ => Ok(None),
        }
    }
    ...
    fn compile_function(
        ...
        for (i, param_val) in function.get_param_iter().enumerate() {
            if let Some(param_info) = params.get(i) {
                ...
                self.variables
                    .insert(param_info.name.clone(), param_val.into());
            }
        }
    ...
    fn compile_yadi(
        ...
        sthiti: &ASTNode,
        tarhi: &[ASTNode],
        anyatha: Option<&[ASTNode]>,
    ) -> Result<Option<BasicValueEnum<'ctx>>, DevvaniLLVMError> {
        let cond = self
            .compile_node(sthiti)?
            .ok_or_else(|| DevvaniLLVMError::CodeGenError("Yadi: no sthiti".into()))?
            .into_int_value();
        let parent = self
            .builder
            .get_insert_block()
    ...
    fn compile_yavat(
        ...
        let cond = self
            .compile_node(sthiti)?
            .ok_or_else(|| DevvaniLLVMError::CodeGenError("Yavat: no sthiti".into()))?
            .into_int_value();
        self.builder
    ...
    fn compile_punah(
        ...
        let n = self
            .compile_node(varam)?
            .ok_or_else(|| DevvaniLLVMError::CodeGenError("Punah: no varam".into()))?
            .into_int_value();
        let parent = self
            .builder
            .get_insert_block()
        ...
        let i_ptr = self
            ...
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        self.builder
            .build_store(i_ptr, self.context.i64_type().const_int(0, false))
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        ...
        let i_val = self
            .builder
            .build_load(i_ptr, "i_load")
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?
            .into_int_value();
        let cond = self
            .builder
            .build_int_compare(inkwell::IntPredicate::SLT, i_val, n, "lttmp")
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        self.builder
            .build_conditional_branch(cond, body_bb, after_bb)
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        ...
        let next_i = self
            .builder
            .build_int_add(i_val, self.context.i64_type().const_int(1, false), "nexti")
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        self.builder
    ...
    fn compile_scanf(&mut self) -> Result<BasicValueEnum<'ctx>, DevvaniLLVMError> {
        ...
        let dest = self
            .builder
            .build_alloca(self.context.i64_type(), "scanftmp")
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        let fmt_ptr = self
            .builder
            .build_global_string_ptr("%lld", "fmtscan")
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        self.builder
            .build_call(
                scanf,
                &[fmt_ptr.as_pointer_value().into(), dest.into()],
                "scanftmpcall",
            )
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?;
        Ok(self
            .builder
            .build_load(dest, "scanload")
            .map_err(|e| DevvaniLLVMError::CodeGenError(e.to_string()))?)
    }
    ...
    fn compile_function_call(
        ...
        let function = self
            .module
            .get_function(name)
            .ok_or_else(|| DevvaniLLVMError::CodeGenError(format!("Unknown function: {}", name)))?;
        let mut compiled_args = Vec::new();
        for arg in args {
            let val = self.compile_node(arg)?.ok_or_else(|| {
                DevvaniLLVMError::CodeGenError("FunctionCall arg has no value".into())
            })?;
