...
impl<'ctx> TypeMapper<'ctx> {
    ...
    pub fn devvani_type_to_llvm(
        ...
        match devvani_type {
            DevvaniType::Vaak => Ok(self.context.i8_type().ptr_type(AddressSpace::default()).into()),
            DevvaniType::VaakBorrow => Ok(self.context.i8_type().ptr_type(AddressSpace::default()).into()),
            _ => Err(DevvaniLLVMError::TypeMapError(format!("Unsupported DevvaniType for LLVM: {:?}", devvani_type))),
        }
    }
}
